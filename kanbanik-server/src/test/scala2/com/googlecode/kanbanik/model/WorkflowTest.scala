...
class WorkflowTest extends FlatSpec with BeforeAndAfter {
  ...
  "removeItem() nested" should "be able to remove item in first level" in {
    val resFirst = workflow.removeItem(item1)
    assert(resFirst.workflowitems === List(item2, item3))

    val resLast = workflow.removeItem(item3)
  ...
  "addItem() second level" should "be able to add at the beginning" in {
    ...
    val res = workflow_1.addItem(itemToAdd, Some(item1), workflow)
  ...
  it should "be able to add at the end" in {
    val itemToAdd = mkWorkflowItem(new ObjectId(), Workflow())
    val res = workflow_1.addItem(itemToAdd, None, workflow)
  ...
  it should "be able to add at the middle" in {
    val itemToAdd = mkWorkflowItem(new ObjectId(), Workflow())
    val res = workflow_1.addItem(itemToAdd, Some(item2), workflow)
  ...
  def mkWorkflowItem(id: ObjectId, nestedWorkflow: Workflow) =
    new Workflowitem(Some(id), "", -1, "", 1, nestedWorkflow, null)
}